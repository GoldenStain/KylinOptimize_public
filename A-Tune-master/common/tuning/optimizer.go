...
func CheckServerPrj(data string, optimizer *Optimizer) error {
	...
	for idx, prj := range prjs {
		...
		// 新增：检查系统是否支持参数
		var supportedObjects []project.Object
		for _, obj := range prj.Object {
			if isSupported(obj.Name) {
				supportedObjects = append(supportedObjects, obj)
			} else {
				log.Warnf("parameter %s is not supported on this system, skipping", obj.Name)
			}
		}
		prj.Object = supportedObjects
...
// isSupported: 检查系统是否支持参数
func isSupported(param string) bool {
